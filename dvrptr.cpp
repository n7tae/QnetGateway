...
// Gorlay API
__inline unsigned int gorlay_encode23127(unsigned int data) 
{
...
static int read_config(const char *cfgFile)
{
   ...
   while (fgets(inbuf, 1020, cnf) != NULL)
   {
      ...
      *p = '\0';
      ...
      if (strcmp(inbuf,"DVRPTR_SERIAL") == 0)
      {
      ...
      if (strcmp(inbuf,"DVRPTR_INTERNAL_PORT") == 0)
      {
      ...
      if (strcmp(inbuf,"DVRPTR_INTERNAL_IP") == 0)
      {
         ptr = strchr(p + 1, ' ');
         if (ptr)
            *ptr = '\0';
      ...
      if (strcmp(inbuf,"GATEWAY_PORT") == 0)
      {
      ...
      if (strcmp(inbuf,"RF_AUDIO_Level") == 0)
      {
...
static void readFrom20000()
{
   ...
   while (true)
   {
      ...
      {
         len = recvfrom (insock, (char *)&recv_buf, 58, 0, (struct sockaddr *)&from, &fromlen);
         if (len == 58) 
         {
            if (busy20000)
            {
	    ...
	    Send_Modem_Header[7] =0x00;
            ...
            if (recv_buf.rf_hdr.flags[0] != 0x01)
            {
               ...
               if (recv_buf.rf_hdr.flags[0] == 0x08)
                  recv_buf.rf_hdr.flags[0] = 0x48;
               else
               if (recv_buf.rf_hdr.flags[0] == 0x20)
                  recv_buf.rf_hdr.flags[0] = 0x60;
               else
               if (recv_buf.rf_hdr.flags[0] == 0x28)
...
bool check_serial()
{
  ...
  for (i = 0; i < 32; i++)
  {
     ...
     while (true)
     {
        if (InitCount == 4 ) {write(fd_ser, Modem_SERIAL, sizeof (Modem_SERIAL)); InitCount = 0;}
        if (InitCount == 3 ) {write(fd_ser, Modem_Init0, sizeof (Modem_Init0)); InitCount = 4;}
        if (InitCount == 2 ) {write(fd_ser, Modem_Init1, sizeof (Modem_Init1)); InitCount = 3;}
        if (InitCount == 1 ) {write(fd_ser, Modem_Init2, sizeof (Modem_Init2)); InitCount = 2;}
...
int main(int argc, const char **argv) 
{
   ...
   while (true)
   {
      ...
      if ((tNow - S_ctrl_msg_time) > 60)
      {
         ...
         memcpy(S_packet + 10, DVCALL, 8); S_packet[17] = 'S';
         memcpy(S_packet + 18, DVCALL, 8); S_packet[25] = 'S';
      ...
      // init Modem  
      if (InitCount == 4 ) {write(fd_ser, Modem_STATUS, sizeof (Modem_STATUS)); InitCount = 0;}
      if (InitCount == 3 ) {write(fd_ser, Modem_Init0, sizeof (Modem_Init0)); InitCount = 4;}
      if (InitCount == 2 ) {write(fd_ser, Modem_Init1, sizeof (Modem_Init1)); InitCount = 3;}
      if (InitCount == 1 ) {write(fd_ser, Modem_Init2, sizeof (Modem_Init2)); InitCount = 2;}
      ...
      if (bytes2 > 0)
      {
            switch (bytes2)
	    {
		  ...
		  strcpy (Temp_Text,"");
	          ...
	          strcpy (Temp_Text,"");
                  ...
                  if (ok)
                  {
                     ...
                     if ((memcmp(myUR, DISABLE_RF, 8) == 0) && (DISABLE_RF[0] != ' '))
                     {
                        IS_ENABLED = false;
                        memcpy(myUR, "CQCQCQ  ", 8);
                     }
                  ...
                  memcpy(S_packet + 18, DVCALL, 8); S_packet[25] = DVRPTR_MOD;
		  ...
		  Send_Network_Header[16] =  0x80; seq_no = 0;
		  ...
		  calcPFCS(Send_Network_Header);
                  ...
                  if (ok)
                  {
                     if (IS_ENABLED)
                     {
                        sendto(insock, (char *)&Send_Network_Header, 58, 0, (struct sockaddr *)&outaddr, sizeof(outaddr));
                        C_COUNTER ++;
                     }
                  }
		  ...
		  memcpy(Send_Network_Audio + 17 , puffer + 8, 12);
                  ...
                  if (IS_ENABLED)
                  {
                     sendto(insock, (char *)&Send_Network_Audio, 29, 0, (struct sockaddr *)&outaddr, sizeof(outaddr));
           ...
           ((last_RF_time > 0) && ((tNow - last_RF_time) > 1)) )
      {
	    ...
	    Send_Network_Header[19] =  0xc8;
            ...
            if (ok)
            {
               if (IS_ENABLED)
               {
	          sendto(insock, (char *)&Send_Network_Header, 29, 0, (struct sockaddr *)&outaddr, sizeof(outaddr));
                  C_COUNTER ++;
               }
            }
